	...
	(
		...
		function ( obj ) 
		{
			...
			if(cdm.id)
			{
				...
				assignTextureAllChildrensLoadPop(obj3D, obj, cdm.material);
			}
			else
			{
			...
			//obj3D.userData.freeze = clickActionBreak_2( obj3D ); 
...
{ 
	var obj = new THREE.Mesh( new THREE.Geometry(), new THREE.MeshLambertMaterial({ color : 0xffff00, transparent: true, opacity: 0.0, side: THREE.DoubleSide }) );
...
function assignTextureAllChildrensLoadPop(obj3D, obj, mat)
{
	...
	{		
		...
		{  
			...
			if(mat[index].type) { inf.type = mat[index].type; }		
			
			loadPopObj_1(inf);	 					
		}
		else
		{
			if(childrens[i].obj.material)
			{ 
				if(new RegExp('mirror','i').test( childrens[i].obj.material.name )) 
				{  
					var color_2 = childrens[i].obj.material.color.clone();			
					childrens[i].obj.material = new THREE.MeshPhongMaterial( { color: color_2, envMap: cubeCamera.renderTarget, lightMap : lightMap_1 } );	
				}								
			}			
		}
	}	
	...
	{		
		...
		{   
			if(components[index].lots) 
			{
				flag = false;
				childrens[i].obj.userData.material = { containerID : containerID[index] };
				var inf = { obj: childrens[i].obj, lotid: components[index].lots[0], start : 'new', containerID : containerID[index] };		
...
{
	if(!obj) return [];
	if(obj.geometry) arr[arr.length] = { obj : obj, name : obj.geometry.name, material : obj.material.name };
...
function loadPopMaterial(cdm, json) 
{
	...
	if(cdm.replace)
	{
		...
		{
			if(obj.userData.parent.userData.tag == 'obj')
			{
				clickO.obj = obj.userData.parent;
			}
		}
